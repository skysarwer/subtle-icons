...
import { getCachedIcons, setCachedIcons, isInflight, markInflight, unmarkInflight } from './useIconCache';

const IconPickerModal = ({ isOpen, onClose, onSelect, initialValue, initialSlug = '', onSelectSlug }) => {
	...
	useEffect( () => {
		...
		if ( isIconifySlug( initialSlug ) ) {
			...
			setSvgDraft( '' );
		} else if ( ( ! initialSlug || initialSlug === 'custom' ) && initialValue?.startsWith( '<svg' ) ) {
	...
	const fetchInitialMetadata = async () => {
		...
		try {
            ...
            if (!debouncedQuery) {
                ...
                if (cached) {
                    setAllIcons(cached);
                    setTotalPages(Math.ceil(cached.length / ICONS_PER_PAGE));
            ...
            // BRANCH B: SEARCH MODE (Server-side Pagination)
                ...
                const response = await apiFetch({
                    path: `/subtle-icons/v1/icons?search=${encodeURIComponent(debouncedQuery)}&page=1&per_page=${ICONS_PER_PAGE}`,
                });
                if (response) {
	...
	const commitSvg = useCallback( ( raw ) => {
		...
		setSelection( clean.includes( '<svg' ) ? { slug: 'custom', svg: clean } : { slug: '', svg: '' } );
	}, [] );
	...
	const handleInsert = async () => {
		...
		if ( isIconifySlug( slug ) ) {
			...
			try {
				...
				if ( iconData ) {
					const { body, width = 24, height = 24, left = 0, top = 0 } = iconData;
	...
	const renderPagination = () => {
        ...
        if (start > 1) {
             pages.push(
                <Button key="first" isSmall variant="tertiary" onClick={() => scrollToPage(1)}>1</Button>
            );
        ...
        if (end < totalPages) {
            if (end < totalPages - 1) pages.push(<span key="dots2" style={{padding: '0 4px'}}>...</span>);
             pages.push(
                <Button key="last" isSmall variant="tertiary" onClick={() => scrollToPage(totalPages)}>{totalPages}</Button>
            );
        }
		...
		const paginationObserver = new IntersectionObserver( ( entries ) => {
			...
			visible.sort( ( a, b ) => {
				const centre = ( e ) => {
					...
					return Math.abs( ( rb.top + rb.height / 2 ) - ( e.boundingClientRect.top + e.boundingClientRect.height / 2 ) );
				};
				return centre( a ) - centre( b );
			} );
			const page = parseInt( visible[ 0 ].target.getAttribute( 'data-page' ), 10 );
		...
		>
			<div className="sbtl-icon-picker-layout">
				<div className="sbtl-icon-picker-sidebar">
					<button
						className={`sbtl-icon-picker-category-btn ${activeCategory === 'general' ? 'is-active' : ''}`}
						onClick={() => setActiveCategory('general')}
					>
						{__('General', 'subtle-icons')}
					</button>
					<button
						className={`sbtl-icon-picker-category-btn ${activeCategory === 'custom' ? 'is-active' : ''}`}
				...
				<div className="sbtl-icon-picker-content">
					{activeCategory !== 'custom' ? (
						<>
							<div className="sbtl-icon-picker-search-container">
								<TextControl
									...
									placeholder={__('Search icons... (comma separated for multiple)', 'subtle-icons')}
								/>
							</div>
							...
							{ isBrowse ? (
								...
								isInitialLoading ? (
									<div className="sbtl-icon-picker-grid-wrapper" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
										<Spinner />
									</div>
								) : (
									<BrowseGrid
								...
								<>
									<div className="sbtl-icon-picker-grid-wrapper" ref={ scrollContainerRef }>
						...
						<p>{ __( 'Paste your custom SVG code below. Ensure it has a viewBox attribute.', 'subtle-icons' ) }</p>
						<textarea
							ref={ textareaRef }
							...
							onPaste={ () => requestAnimationFrame( () => commitSvg( textareaRef.current?.value ?? '' ) ) }
								placeholder="<svg...>"
							/>
						</div>
					)}
			...
			<div className="sbtl-icon-picker-footer">
				<Button variant="link" isDestructive onClick={ () => { setSelection( { slug: '', svg: '' } ); setSvgDraft( '' ); } }>
                ...
                <div className="sbtl-icon-picker-footer__right">
                            ...
                            <Button
                                ...
                                onClick={ () => {
                                    ...
                                    if ( isIconifySlug( selection.slug ) ) {
                                        const iconData = getIcon( selection.slug );
                                        if ( iconData ) {
                                            const { body, width = 24, height = 24, left = 0, top = 0 } = iconData;
                    ...
                    { isIconifySlug( selection.slug ) ? (
                        <span className="sbtl-icon-preview"><Icon icon={ selection.slug } /></span>
                    ) : selection.svg ? (
                        <span className="sbtl-icon-preview" dangerouslySetInnerHTML={{ __html: sanitizeSvg( selection.svg ) }} />
                    ) : (
                    ...
                    >
                        {isInserting ? __('Inserting...', 'subtle-icons') : __('Insert', 'subtle-icons')}
                    </Button>
                </div>
			</div>
		</Modal>
