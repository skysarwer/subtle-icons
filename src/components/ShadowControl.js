...
const parseShadowLayer = ( layer ) => {
	...
	const [ , x, unitX, y, unitY, blur, unitBlur, spread, unitSpread, color ] =
		match;
	const unit =
		unitX || unitY || unitBlur || unitSpread || DEFAULT_SHADOW_LAYER.unit;
...
const buildShadowValue = ( layers ) =>
	layers
		.map( ( layer ) => {
			...
			const x = Number.isFinite( layer.x )
				? layer.x
				: DEFAULT_SHADOW_LAYER.x;
			const y = Number.isFinite( layer.y )
				? layer.y
				: DEFAULT_SHADOW_LAYER.y;
			const blur = Number.isFinite( layer.blur )
				? layer.blur
				: DEFAULT_SHADOW_LAYER.blur;
...
const ShadowControl = ( {
	...
	const orderedShadowPresets = useMemo( () => {
		...
		return [ ...presets, ...modifiedPresets ];
	...
	useEffect( () => {
		...
		if ( isCustomShadowValue ) {
			setShadowLayers(
				splitShadowLayers( valToUse ).map( parseShadowLayer )
			);
		} else {
			setShadowLayers( EMPTY_ARRAY );
		}
	...
	const updateLayer = ( index, patch ) => {
		const baseLayers = shadowLayers.length
			? shadowLayers
			: [ { ...DEFAULT_SHADOW_LAYER } ];
	...
	return (
		<>
			...
			{ isPopoverVisible && (
				<Popover
					className="sbtl-shadow-popover"
					position="center left"
					onClose={ togglePopover }
				>
					<BaseControl
						className="sbtl-shadow-popover--base"
						label={ label }
						help={ help }
					>
						{ viewMode === 'presets' && (
											...
											<>
												...
												<div className="sbtl-shadow-builder__layer">
													...
													<Grid
														columns={ 2 }
														gap={ 4 }
														className="sbtl-shadow-builder__grid"
													>
