...
export default function Edit( {
	...
	useEffect( () => {
		if (
			! icon &&
			! disabledDefaultIcon &&
			window.sbtl_icon_text_icons?.info
		) {
		...
		<>
				...
				>
					<LinkControl
						...
						help={
							! url
								? __(
										'Make the icon and text a single clickable link.',
										'subtle-icons'
								  )
								: undefined
						}
			...
			<InspectorControls group="styles">
				...
				>
					<ToolsPanelItem
						...
						hasValue={ () => !! shadow }
						onDeselect={ () =>
							setAttributes( { shadow: undefined } )
						}
					...
					>
						<ShadowControl
							...
							value={ shadow }
							onChange={ ( value ) =>
								setAttributes( { shadow: value || undefined } )
							}
						/>
					</ToolsPanelItem>
				</ToolsPanel>
			...
			<TagName { ...blockProps }>
				<RegionTagName
					...
					href={ url ? '#' : undefined }
					target={ url ? linkTarget || undefined : undefined }
				...
				>
					...
					<RichText
						...
						placeholder={
							placeholder || __( 'Write text…', 'subtle-icons' )
						}
						value={ content }
						onChange={ ( value ) =>
